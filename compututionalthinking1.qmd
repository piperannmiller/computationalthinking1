...
##named function ,only one arugment, adding one to x, calling object output, print output 
add_one  <- function(x){
...
```{r} 
##adding our function to 10, which is x+1, so its 10+1=11
...
#non numerical argument when it is five instead of 5
...
```{r}
add_together
#be able to see the function and see the source code easily 
```
...
```{r}
# Create a new function called math_time
# 3 arguments, x, y, z
##named function ,only one arugment, adding one to x, calling object output, print output 
...
lbs_to_kg  <- function(weights){
  ...
  # Print out whatever is stored in "output"
  return(output)
  #made a function called "weights"
} 
```
...
#vector/values of bison weights 
```
...
#putting the values into the function to get the correct output and conversion 
...
```{r}
#making the function, have to have curly brackets!!!!
...
##A2.1: Measurements are in centimeters 
...
#finding the mean of each column by writing all columns 
```
...
```{r}
#using across() to quickly get the mean for all columns without writing out all columns 
iris %>%
  group_by(Species) %>% 
...
```{r}
#use : to mean blank column to blank column 
iris %>%
  group_by(Species) %>% 
...
iris %>%
  ...
  summarize(across(.cols = everything(), 
                   .fns = median))
#change the fns to median, I used the median function
```
...
```{r}
#select all columns that are numerical to find their mean 
iris %>%
  group_by(Species) %>% 
...
#add period and backslash before data
cereal %>%
  group_by(mfr) %>% 
  summarize(across(.cols = where(is.numeric), 
                   .fns = mean))
#mean for all numerical columns and values 
...
```{r}
#printed out numbers 1 thru 5 then multiplied each number by 2
for (i in 1:5) {
  print(i*2)
}
```
...
```{r}
#vector we created, stores pop sizes, havent filled in the rest, 100 is intital 
print(N)
```
...
```{r}
N
#filled in the values using the loop 
```
...
#plotted years vs population size 
```
...
#plotted years vs population size 
#new graph with new parameters, the population is expontientally decreasing, it reaches 0 by the end of 50 years. 
```
...
iris %>% 
  summarize(across(.cols = 1:4, 
                   .fns = mean))
#mean for each column, using across function 
```

### Q2.6 Which do you prefer?

#A2.6: I prefer the across function, I find it more straightforward and easier to use and find error in. I think it also takes less steps than the for loop function. 
...
# creating the for loop and stating 
for (i in 1:length(spp_names)) {
  ...
  # showing the plot on the terminal 
  print(plot)
}
